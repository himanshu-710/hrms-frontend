...
export default function ExperienceSection() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      onboardingApi.getExperience(employeeId as number),
    enabled: !!employeeId,
  });

 
  const experienceList: ExperienceItem[] = Array.isArray(data)
    ? data
    : [];

  const addMutation = useMutation({
    mutationFn: (values: ExperiencePayload) =>
      onboardingApi.createExperience(values),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      onboardingApi.deleteExperience(id),
